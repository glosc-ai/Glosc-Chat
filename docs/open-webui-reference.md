...
| 模型连接 | 面向多模型和兼容接口 | 先做 Chat Completions 兼容，再扩展 |
...
| Chat Completions 兼容接口 | 兼容接口能覆盖大量模型网关 | MVP 优先支持 Chat Completions 兼容 |
...
### DDR-001：MVP 优先 Chat Completions 兼容

决策：MVP 只强制支持 Chat Completions 兼容 Provider。
...
- Provider Adapter 必须设计为可扩展，避免和 Chat Completions payload 绑死。
