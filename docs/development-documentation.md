...
| `yarn build` | 运行 `vue-tsc --noEmit` 并构建前端 |
| `yarn preview` | 预览 Vite 构建结果 |
| `yarn tauri` | 调用 Tauri CLI |
...
当前 `src/App.vue` 已替换为 Glosc Chat 移动端优先界面，覆盖聊天、模型、Provider 和设置主流程。领域类型、种子数据、聊天服务和本地存储已拆分到 `src/domain/`、`src/data/` 和 `src/services/`，后续可继续把大型视图拆成组件。
